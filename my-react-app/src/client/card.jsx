  ...
  return (
    <div className="relative group duration-500 cursor-pointer overflow-hidden text-gray-50 h-72 w-56 rounded-2xl hover:duration-700 duration-700">
      ...
      <div className="absolute bg-gray-50 -bottom-24 w-56 p-3 flex flex-col gap-1 group-hover:-bottom-0 group-hover:duration-600 duration-500">
        <span className="text-lime-400 font-bold text-xs">{props.language}</span>
        <span className="text-gray-800 font-bold text-3xl">{props.title}</span>
        <p className="text-neutral-800">{props.codedetails}</p>
      </div>
    </div>
  );
}
